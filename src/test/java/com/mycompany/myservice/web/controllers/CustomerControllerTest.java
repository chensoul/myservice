...
class CustomerControllerTest {
	...
	void shouldFetchAllCustomers() throws Exception {
		...
		Page<Customer> page = new Page<>(0, 10, 3);
	...
	void shouldFindCustomerById() throws Exception {
		...
		this.mockMvc.perform(get("/api/customers/{id}", customerId))
			.andExpect(status().isOk())
	...
	void shouldUpdateCustomer() throws Exception {
		...
		given(customerService.updateCustomer(eq(customerId), any(CustomerRequest.class))).willReturn(customer);

		this.mockMvc
			.perform(put("/api/customers/{id}", customerId).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(customerRequest)))
			.andExpect(status().isOk())
